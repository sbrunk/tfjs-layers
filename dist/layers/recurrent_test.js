...
test_utils_1.describeMathCPUAndGPU('RNN-Layer-Math', function () {
    ...
    it('call: 1 state: returnSequences=false, returnState=false', function () {
        ...
        test_utils_1.expectTensorsClose(outputs, K.scalarTimesArray(tfjs_core_1.scalar(-57.75), tfc.ones([2, 4])));
    });
    ...
    it('call: with 1 initialState', function () {
        ...
        test_utils_1.expectTensorsClose(outputs, K.scalarTimesArray(tfjs_core_1.scalar(-58.75), tfc.ones([2, 4])));
    });
    it('call: with 2 initialStates', function () {
        ...
        test_utils_1.expectTensorsClose(outputs[1], K.scalarTimesArray(tfjs_core_1.scalar(58.75), tfc.ones([2, 4])));
        test_utils_1.expectTensorsClose(outputs[2], K.scalarTimesArray(tfjs_core_1.scalar(59.75), tfc.ones([2, 5])));
    });
...
test_utils_1.describeMathCPUAndGPU('SimpleRNN Tensor', function () {
    ...
    var _loop_1 = function (activation) {
        ...
        it(testTitle, function () {
            ...
            test_utils_1.expectTensorsClose(output, K.scalarTimesArray(tfjs_core_1.scalar(expectedElementValue), tfc.ones([batchSize, units])));
        });
    };
    ...
    var _loop_2 = function (returnState) {
        ...
        it(testTitle, function () {
            ...
            test_utils_1.expectTensorsClose(outputT0, K.scalarTimesArray(tfjs_core_1.scalar(inputSize + 1), tfc.ones([1, batchSize, units])));
            test_utils_1.expectTensorsClose(outputT1, K.scalarTimesArray(tfjs_core_1.scalar((inputSize + 1) * (units + 1)), tfc.ones([1, batchSize, units])));
            if (returnState) {
    ...
    it('BPTT', function () {
        ...
        test_utils_1.expectTensorsClose(simpleRNN.getWeights()[0], K.scalarTimesArray(tfjs_core_1.scalar(0.8484658), tfc.ones([4, 1])));
        test_utils_1.expectTensorsClose(simpleRNN.getWeights()[1], K.scalarTimesArray(tfjs_core_1.scalar(0.8484799), tfc.ones([1, 1])));
        test_utils_1.expectTensorsClose(dense.getWeights()[0], K.scalarTimesArray(tfjs_core_1.scalar(80.967026), tfc.ones([1, 1])));
    });
});
        ...
        var _loop_4 = function (returnState) {
            var _loop_5 = function (returnSequences) {
                ...
                it(testTitle, function () {
                    ...
                    var expectedOutput;
                    if (returnSequences) {
                        var outputs = goldenOutputElementValues.map(function (value) { return K.scalarTimesArray(tfjs_core_1.scalar(value), tfc.ones([1, batchSize, units])); });
                        expectedOutput = tfc.transpose(K.concatAlongFirstAxis(K.concatAlongFirstAxis(outputs[0], outputs[1]), outputs[2]), [1, 0, 2]);
                    }
                    else {
                        expectedOutput = K.scalarTimesArray(tfjs_core_1.scalar(goldenOutputElementValueFinal), tfc.ones([batchSize, units]));
                    }
                    if (returnState) {
                        output = output;
                        expect(output.length).toEqual(2);
                        test_utils_1.expectTensorsClose(output[0], expectedOutput);
                        test_utils_1.expectTensorsClose(output[1], K.scalarTimesArray(tfjs_core_1.scalar(goldenOutputElementValueFinal), tfc.ones([batchSize, units])));
                    }
                    else {
                        output = output;
        ...
        var _loop_7 = function (returnState) {
            var _loop_8 = function (returnSequences) {
                ...
                it(testTitle, function () {
                    ...
                    var expectedOutput;
                    if (returnSequences) {
                        var outputAtT0 = K.scalarTimesArray(tfjs_core_1.scalar(goldenOutputElementValueAtT0), tfc.ones([1, batchSize, units]));
                        var outputAtT1 = K.scalarTimesArray(tfjs_core_1.scalar(goldenOutputElementValueAtT1), tfc.ones([1, batchSize, units]));
                        expectedOutput = tfc.transpose(K.concatAlongFirstAxis(outputAtT0, outputAtT1), [1, 0, 2]);
                    }
                    else {
                        expectedOutput = K.scalarTimesArray(tfjs_core_1.scalar(goldenOutputElementValueAtT1), tfc.ones([batchSize, units]));
                    }
                    if (returnState) {
                        output = output;
                        expect(output.length).toEqual(3);
                        test_utils_1.expectTensorsClose(output[0], expectedOutput);
                        test_utils_1.expectTensorsClose(output[1], K.scalarTimesArray(tfjs_core_1.scalar(goldenHStateElementValue), tfc.ones([batchSize, units])));
                        test_utils_1.expectTensorsClose(output[2], K.scalarTimesArray(tfjs_core_1.scalar(goldenCStateElementValue), tfc.ones([batchSize, units])));
                    }
                    else {
                        output = output;
                        test_utils_1.expectTensorsClose(output, expectedOutput);
