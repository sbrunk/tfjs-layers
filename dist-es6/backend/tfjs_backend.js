...
export function normalizeBatchInTraining(x, gamma, beta, reductionAxes, epsilon) {
    if (epsilon === void 0) { epsilon = 1e-3; }
    if (util.arraysEqual(reductionAxes.slice().sort(), math_utils.range(0, ndim(x) - 1))) {
...
export function l2Normalize(x, axis) {
    return tidy(function () {
        ...
        var epsilonTensor = scalarTimesArray(scalar(epsilon()), tfc.onesLike(x));
...
export function binaryCrossentropy(target, output, fromLogits) {
    if (fromLogits === void 0) { fromLogits = false; }
    return tidy(function () {
        var y;
        if (!fromLogits) {
...
export function hardSigmoid(x) {
    return tidy(function () {
        var y = scalarPlusArray(scalar(0.5), scalarTimesArray(scalar(0.2), x));
